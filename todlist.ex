...
defmodule ToDoList do
  ...
  def entries(todolist,date)do
    entries = todolist.entries |> Stream.filter(fn {_,entry} -> entry.date == date end ) |> Enum.map(fn {_,entry} ->
      entry
      end
    )
    entries
...
defmodule ToDoList.CsvImporter do
  ...
  def match_dates([datestring,title])do
    ...
    {_, date_entry} = Date.new(String.to_integer(year), String.to_integer(month), String.to_integer(date))
